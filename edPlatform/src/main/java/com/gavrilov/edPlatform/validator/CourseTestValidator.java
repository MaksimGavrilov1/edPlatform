...
public class CourseTestValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        rejectIfEmptyOrWhitespace(errors, "name", "", ValidationUtils.NOT_EMPTY_THEME_TEST_NAME);
        ...
        for (int i = 0; i < courseTest.getTestQuestions().size(); i++) {
            ...
            if (question.getText().trim().isBlank()) {
                errors.rejectValue("testQuestions", "", String.format(ValidationUtils.NOT_EMPTY_QUESTION_TEXT_MODIFYING, i+1));
            }
            rightAnswerIter = 0;
            ...
            for (int j = 0; j < question.getQuestionStandardAnswers().size(); j++) {
                ...
                if (answer.getText().isBlank()) {
                    errors.rejectValue("testQuestions", "", String.format(ValidationUtils.NOT_EMPTY_ANSWER_TEXT_MODIFYING, j+1, i+1));
                ...
                //check if list with repeatable answer contains an answer
                        ...
                        if (repeatedAnswers.size() != 0) {
                            errors.rejectValue("testQuestions", "", String.format(ValidationUtils.REPEATABLE_ANSWER_MODIFYING, i+1, j+1, repeatedAnswers.size()));
            ...
            if (rightAnswerIter == 0) {
                errors.rejectValue("testQuestions", "", String.format(ValidationUtils.NO_RIGHT_ANSWER_MODIFYING, i+1));
            }
        }
    }
}
